...
shinyServer(function(input, output, session) {
  ...
  get.dat <- reactive({
    ...
    df <- read.table(input$file1$datapath,sep = ",", header = TRUE)
    dataset <- df#input$dataset
  ...
  render.summary <- reactive({
    ...
    summary$data <- c(df, input$compound)  #input$dataset
  ...
  observe({
    df<-data.frame(df)
    comp <- as.vector(unique(df$compound))#input$dataset
...
#  })
  ...
  observe({
    #result.plot <<- render.plot()
    #result.summary <<- render.summary()
    
    output$download_all <- downloadHandler(
      filename ="report.pdf",
      content = function(file) {
        rmd <- rmarkdown::render(input="report.Rmd")
        file.rename(rmd, file)
      }
    )
